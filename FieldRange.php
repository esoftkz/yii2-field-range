...
class FieldRange extends \yii\base\Widget
{
    ...
    protected function getFormInput()
    {
		...
		if(!empty($this->metric))
			$this->metric = '<span class="metric kv-field-separator input-group-addon"> '.$this->metric.' </span>';
		
		
        return $input1 . '<span class="input-group-addon kv-field-separator">' . $this->separator . '</span>' . $input2 . $this->metric;
    }
